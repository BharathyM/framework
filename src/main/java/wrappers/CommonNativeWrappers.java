        ...
        try {
            ...
            if(platformName.equalsIgnoreCase("Android")){
              ...
              /*  options.setCapability("uiautomator2ServerInstallTimeout", 120000);
                ...
                options.setCapability("uiautomator2ServerReadTimeout", 300000);*/
                ...
                Thread.sleep(3000);
                System.out.println("IOS app is launched!");
            }
...
//					return getDriver().findElement(AppiumBy.name(locValue));
    ...
    public void zoomInApp() {
        int maxY = getDriver().manage().window().getSize().getHeight();
        int maxX = getDriver().manage().window().getSize().getWidth();
        PointerInput finger1 = new PointerInput(Kind.TOUCH, "bharathy-finger1");
        Sequence a = new Sequence(finger1, 1);
        ...
        a.addAction(finger1.createPointerMove(Duration.ofSeconds(1), Origin.viewport(), (int) (maxX * 0.75),
                (int) (maxY * 0.25)));
        a.addAction(finger1.createPointerUp(MouseButton.LEFT.asArg()));
        PointerInput finger2 = new PointerInput(Kind.TOUCH, "bharathy-finger2");
        Sequence b = new Sequence(finger2, 1);
    ...
    public WebElement selectElementUsingText(String text) {
    ...
    public void add_Log_With_ScreenShot(String message) throws IOException {
    ...
    public static  void getBase64Screenshot() throws IOException {
        String Screenshot =  ((TakesScreenshot) getDriver()).getScreenshotAs(OutputType.BASE64);
        ExtentCucumberAdapter.getCurrentStep().log(Status.INFO, MediaEntityBuilder.createScreenCaptureFromBase64String(Screenshot).build().toString());
    }
}
